...
// entityState_t->event values (ioquake code/game/bg_public.h), required for local G_AddEVent
...
typedef struct {
    ...
    int cursize;         // +0x10
    int readcount;       // +0x14
    int bit;             // +0x18  bit position for bitwise reads/writes
    uint8_t _padd[4];
} msg_t;                 // 0x20 bytes


typedef struct ALIGN(4) usercmd_s {
    int serverTime;     // +0
    int angles[3];      // +4
    int buttons;        // +16
    byte weapon;        // +20
...
typedef struct ALIGN(8) pmove_s { 
    playerState_t *ps;
    usercmd_t cmd;
    int tracemask;
    int debugLevel;
...
// client_t - 154.468 bytes (3,860 less than x64). Verified from quakelive_steam.exe build 1069.
// Layout reconstructed from SV_SendServerCommand and svs struct layout
// audit patterns functions via SV_SendMessageToClient
typedef struct client_s {
    clientState_t state;                                // +0 (win32)
    char userinfo[MAX_INFO_STRING];                     // +4 (win32) (1024 bytes)

    char reliableCommands[MAX_RELIABLE_COMMANDS][MAX_STRING_CHARS]; // +1028 (64*1024=65536)
    int reliableSequence;                               // +66564 (win32)
    int reliableAcknowledge;                            // +66568 (win32)
    int reliableSent;                                   // +66572 (win32)
    int messageAcknowledge;                             // +66576 (win32)

    int gamestateMessageNum;                            // +66580 (win32)
    int challenge;                                      // +66584 (win32)

    usercmd_t lastUsercmd;                              // +66588 (win32) (28 bytes)
    int lastMessageNum;                                 // +66616 (win32)
    int lastClientCommand;                              // +66620 (win32)
    char lastClientCommandString[MAX_STRING_CHARS];     // +66624 (win32) (1024 bytes)
    sharedEntity_t *gentity;                            // +67648 (win32) (8 bytes, ptr)
    char name[MAX_NAME_LENGTH];                         // +67652 (win32) [QL] 40 bytes (Q3 used 32)

    // --- UDP download fields (vestigial in QL - completely unreferenced) ---
    char downloadName[MAX_QPATH];                       // +67692 (win32) (64 bytes)
    fileHandle_t download;                              // +67752 (win32)
    int downloadSize;                                   // +67756 (win32)
    int downloadCount;                                  // +67760 (win32)
    int downloadClientBlock;                            // +67764 (win32)
    int downloadCurrentBlock;                           // +67768 (win32)
    int downloadXmitBlock;                              // +67772 (win32)
    unsigned char *downloadBlocks[MAX_DOWNLOAD_WINDOW]; // +67776 (win32) (32 bytes)
    int downloadBlockSize[MAX_DOWNLOAD_WINDOW];         // +67808 (win32) (32 bytes)
    qboolean downloadEOF;                               // +67840 (win32)
    int downloadSendTime;                               // +67844 (win32)

    // --- Dead space: unreferenced region, likely remnant of removed features ---
    uint8_t _dead_space[160];                           // +67848 (win32) (reduced space compared with x64)

    int deltaMessage;                                   // +68012 (win32)
    int nextReliableTime;                               // +68016 (win32)
    float floodCount;                                   // +68020 (win32) [QL] token-bucket flood counter
    int lastPacketTime;                                 // +68024 (win32)
    int lastConnectTime;                                // +68028 (win32)
    int nextSnapshotTime;                               // +68032 (win32)
    qboolean rateDelayed;                               // +68036 (win32)
    int timeoutCount;                                   // +68040 (win32)
    clientSnapshot_t frames[PACKET_BACKUP];             // +68044 (win32) (32*648=20736)
    int ping;                                           // +88780 (win32)
    int rate;                                           // +88784 (win32)
    int snapshotMsec;                                   // +88788 (win32)
    int pureAuthentic;                                  // +88792 (win32)
    qboolean gotCP;                                     // +88796 (win32)
    uint8_t _padding_netchan[4];                        // +88804 (win32)
    netchan_t netchan;                                  // +88804 (win32) (65596 bytes)
    netchan_buffer_t *netchan_start_queue;              // 154400 (win32) (4 bytes, ptr) 
    netchan_buffer_t **netchan_end_queue;               // 154404 (win32) (4 bytes, ptr) 

    // --- Post-netchan fields (QL additions) --- 
    uint8_t _reserved[40];                              // +154408 (win32) - unreferenced
    int rageQuit;                                       // +154448 (win32) [QL] set by game module via syscall
    int serverIdSequence;                               // +154452 (win32) [QL] 7-bit server ID tracking 
    int oldServerTime;                                  // +154456 (win32) [QL] map_restart time adjustment
    uint8_t _padding[4];                                // +154460 (win32)
    uint64_t steam_id;                                  // +154464 (win32) [QL] id hint ( 4 + 4 );
} client_t;                                             // 154.468 bytes (x32)
...
// Verified from quakelive_steam.exe build 1069.
typedef struct {
...
//  int _padding;                           // +12 (non existant in win32)
...
// Original gentity_t Struct from MinoMino quake_common.h, validated in win32
struct gentity_s {
  entityState_t s;
  entityShared_t r;
